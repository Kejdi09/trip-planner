  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FCFD' }} edges={['top']}>
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 120 }}>
        ...
        <View style={{ marginTop: 14, backgroundColor: '#fff', borderRadius: 20, borderWidth: 1, borderColor: '#D7EEF0', padding: 16 }}>
          <Text style={{ fontWeight: '800', color: '#0F172A', marginBottom: 8 }}>Travel Stats</Text>
          <Text>Countries: {summary?.countries ?? 0}</Text>
          <Text>Cities: {summary?.cities ?? 0}</Text>
          <Text>Places rated: {summary?.placesRated ?? 0}</Text>
          <Text>Group trips: {summary?.groupTrips ?? 0}</Text>
          <View style={{ marginTop: 8, flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
            {(summary?.visitedCountries ?? []).length === 0 ? <Text style={{ color: '#64748B' }}>No trips yet.</Text> : (summary?.visitedCountries ?? []).map((c) => <View key={c} style={{ backgroundColor: '#D7EDF0', borderRadius: 999, paddingHorizontal: 10, paddingVertical: 6 }}><Text style={{ color: '#0F766E', fontWeight: '700', fontSize: 12 }}>{c}</Text></View>)}
          </View>
        </View>
      </ScrollView>
    </SafeAreaView>
